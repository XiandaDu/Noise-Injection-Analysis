...
for i, (images, labels) in enumerate(tqdm(dataloader)):
    os.makedirs(f"./fgsm_result/image{i+1}-by-depth", exist_ok=True)
    ...
    extractor.clear_features()
    model(adv_images_2)
    save_feature_maps_to_csv_by_depth(extractor.features, os.path.join(output_dir+f"/image{i+1}-by-depth", f"fgsm_features_{i + 1}_e2.csv"))

    extractor.clear_features()
    model(adv_images_4)
    save_feature_maps_to_csv_by_depth(extractor.features, os.path.join(output_dir+f"/image{i+1}-by-depth", f"fgsm_features_{i + 1}_e4.csv"))

    extractor.clear_features()
    model(adv_images_6)
    save_feature_maps_to_csv_by_depth(extractor.features, os.path.join(output_dir+f"/image{i+1}-by-depth", f"fgsm_features_{i + 1}_e6.csv"))

    extractor.clear_features()
    model(adv_images_8)
    save_feature_maps_to_csv_by_depth(extractor.features, os.path.join(output_dir+f"/image{i+1}-by-depth", f"fgsm_features_{i + 1}_e8.csv"))
