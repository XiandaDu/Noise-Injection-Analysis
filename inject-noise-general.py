...
def process_image(image_path, image_label):
    ...
    image = Image.open(f"./Training/{image_path}.JPEG").convert("RGB")
    ...
    save_image_from_tensor(noisy_input_gaussian, f"{image_path}-gaussian-noised.JPEG")
    features.clear()
    with torch.no_grad():
    ...
    save_image_from_tensor(noisy_input_dot, f"{image_path}-dot-noised.JPEG")
    features.clear()
    with torch.no_grad():
...
if __name__ == "__main__":
    image_path = "n01753488_177"
    process_image(image_path, "")



# salt and pepper noise
# more standard data source (with labels)
# *data visualizations (not 1D!!) and statistics(means, median, std) of feature maps, 
# do DCT on feature maps, dimension reduction (PCA)
# Test on more pics on more classes
# Try to increase the randomness until the prediction becomes incorrect. Record the whole process. 
# Try more noise levels
# More types of noises in the future



# image from ImageNet, go through ResNet AI and gather the feature maps of the four layers. 
# Noise type is gaussian noise, dot noise, salt and pepper noise.
# Todos: in the future I could add some more types noise.

# For the noise that I have, I adjusted the extend of the noise. 
# One thing Kaixiang told me: Try to increase the extend of the noise until the prediction becomes incorrect

# Go into data. For data analysis, here is the mean,  median, std, data visualization (histogram)