...
def main():
    ...
    # Only iterate over 2 images per class
    ...
    print("Average FGSM Losses (eps=0.5...5/255):")
    for i, avg_loss in enumerate(fgsm_avg):
        print(f"eps={0.5*i/255:.2f}: {avg_loss:.4f}")

    print("\nAverage Gaussian Noise Losses (sigma=0.2...1.8):")
    for j, avg_loss in enumerate(gauss_avg):
        print(f"sigma={0.2*j:.2f}: {avg_loss:.4f}")

    plt.figure(figsize=(8,4))
    plt.subplot(1,2,1)
    plt.plot([0.5*i for i in range(0,10)], fgsm_avg, marker='o')
    plt.title("FGSM Loss vs. eps=0.5...5/255")
    plt.xlabel("eps (integer, /255)")
    plt.ylabel("CE Loss")

    plt.subplot(1,2,2)
    plt.plot([0.2*i for i in range(0,10)], gauss_avg, marker='o')
    plt.title("Gaussian Loss vs. sigma=0.2...1.8")
    plt.xlabel("sigma")
    plt.ylabel("CE Loss")
    plt.tight_layout()
    plt.show()
